...
public class DisplayBoard extends JPanel implements KeyListener {
    ...
    private boolean hasBorder = false; //border in between squares when painted
    ...
    private void setup() {
        ...
        //frame.setBackground(DEFAULT_COLOR);
        ...
        if (hasBorder) {
            ...
            frame.setPreferredSize(new Dimension((game.getBoard()[0].length)*(scale)+1+2*margin_w,  (game.getBoard().length)*(scale)+23+2*margin_h));
        }
   ...
   //          menu.add(rbMenuItemWrap);
    ...
    public void gameOver() {
        ...
        if (game instanceof Java_2048)
            setBlockLabel();
    	if (auto_restart && !(game instanceof Pacman)) {
            try {
    ...
    public void paintComponent(Graphics g) {
        ...
        g2d.fillRect(0,0,frame_w,frame_h);
        if (game instanceof ImageBoard) {
            String[][] board = ((ImageBoard) game).getImages();
            //paint background
            BufferedImage backgroundImage = allImages.get(((ImageBoard) game).getBackgroundImage());
            if (hasBorder) {
                g2d.drawImage(backgroundImage,0,0,(board[0].length)*(scale+1)+1,  (board.length)*(scale+1)+1, this);
            } else {
                g2d.drawImage(backgroundImage,0,scale,(board[0].length)*(scale)+1,  (board.length-1)*(scale)+1, this);
            }
                ...
                for (int c = 0;c<board[0].length;c++) {
                    ...
                    if (board[r][c] == null) {
                        ...
                        try {
                            ...
                            if (image == null) {
                                // System.out.print("ERROR: FILE NOT LOADED IN - ");
                                // throw new RuntimeException();
                            } else if (hasBorder) {
                                g2d.drawImage(image,c*(scale+1)+margin_w+1, (game.getBoard().length-r-1)*(scale+1)+margin_h+1,scale,scale,this);
                            } else {
                                g2d.drawImage(image,c*(scale)+margin_w+1, (game.getBoard().length-r-1)*(scale)+margin_h+1,scale,scale,this);
                            }
                        } catch (Throwable t) {
            ...
            //now top images
            ArrayList<PositionedImage> topImages = ((ImageBoard)game).getTopImages();
            double x_pos;
            double y_pos;
            double x_size;
            double y_size;
            for (PositionedImage pi : topImages) {
                image = allImages.get(pi.getImage());
                if (pi.getPosition()==null || pi.getSize()==null) {
                    continue;
                }
                x_pos = pi.getPosition().x;
                y_pos = pi.getPosition().y;
                x_size = pi.getSize().x;
                y_size = pi.getSize().y;
                if (image == null) {
                    continue;
                } else if (hasBorder) {
                    g2d.drawImage(image,(int)(x_pos*(scale+1)+1+margin_w), (int)((game.getBoard().length-y_pos-1)*(scale+1)+1+margin_h),(int)(scale*x_size),(int)(scale*y_size),this);
                } else {
                    g2d.drawImage(image,(int)(x_pos*(scale)+1+margin_w), (int)((game.getBoard().length-y_pos-1)*(scale)+1+margin_h),(int)(scale*x_size),(int)(scale*y_size),this);
                }
            }
    ...
    private void setScale() {
        ...
        int scale_2;
        if (hasBorder) {
            scale_1 = (int)((frame_w-2*margin_w-1)/((double)game.getBoard()[0].length)-1);
            scale_2 = (int)((frame_h-2*margin_h-23)/((double)game.getBoard().length)-1);
        } else {
            scale_1 = (int)((frame_w-2*margin_w-1)/((double)game.getBoard()[0].length)-1)+1;
            scale_2 = (int)((frame_h-2*margin_h-23)/((double)game.getBoard().length)-1)+1;
        }
